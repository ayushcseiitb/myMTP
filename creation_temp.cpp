...
// Set up the output stream. 
    std::error_code EC;
    llvm::raw_fd_ostream output ("temp.asm",EC,llvm::sys::fs::OF_None);

// Generate assembly code.
    llvm:: legacy::PassManager passManager;
    targetMachine->addPassesToEmitFile(passManager, output, nullptr,llvm::CGFT_AssemblyFile);
    passManager.run(module);
    
    std::ifstream inFile("temp.asm");
...
// Create a new module to store the assembly code. 
    ...
    llvm::GlobalVariable* AssemblyGlobal = new llvm::GlobalVariable(*assemblyModule,llvm::ArrayType::get(llvm::IntegerType::get(llvm_ctx,8),assembly.size()+1),
                                                                                    true,
                                                                                    llvm::GlobalValue::InternalLinkage,
                                                                                    llvm::ConstantDataArray::getString(llvm_ctx,assembly),
                                                                                    "assembly");
