...
bool AssemblyModule(
    PassManagerBase &PM, llvm::Module &M, llvm::Module &AssemblyModule,
    bool DisableVerify, MachineModuleInfoWrapperPass *MMIWP) {
  // Add common CodeGen passes.
  if (!MMIWP)
    MMIWP = new MachineModuleInfoWrapperPass(M);
  TargetPassConfig *PassConfig =
      addPassesToGenerateCode(*M, PM, DisableVerify, *MMIWP);
  if (!PassConfig)
    return true;

  if (TargetPassConfig::willCompleteCodeGenPipeline()) {
    // Create a pass to generate assembly code and store it in the AssemblyModule.
    PM.add(llvm::createAsmPrinterPass(&AssemblyModule));
  } else {
    // MIR printing is redundant with -filetype=null.
    PM.add(llvm::createPrintMIRPass(&AssemblyModule));
  }
...
// Generate assembly code.
    ...
    targetMachine->AssemblyModule(passManager, module,*assemblyModule, nullptr,llvm::CGFT_AssemblyFile);
...
// Store the assembly language in the module's metadata.
    ...
    llvm::MDNode* assemblyMD = llvm::MDNode::get(llvm_ctx, llvm::MDString::get(llvm_ctx, assembly)); 
